...
<!-- # Physically-based hydrologic modeling using GRASS r.topmodel {.unnumbered} -->
# Welcome {.unnumbered #sec-index}

![](images/NSF_grant_announcement.webp){align=center width=75%}
...
![](images/subbasins-for-r.topmodel.webp){.align-center width=75%}

## Abstract

The Topography Model (TOPMODEL) is "a set of conceptual tools that can be used to reproduce the hydrological behaviour of catchments in a distributed or semi-distributed way, in particular the dynamics of surface of subsurface contributing areas" (Beven et al. 1995). Cho (2000) reimplemented his FORTRAN code as a GRASS module in C, based on which the R package (Buytaert 2009) and SAGA GIS module (Conrad 2003) have been developed (Cho et al. 2019). Cho (2020) developed r.accumulate, an efficient GRASS hydrologic module for calculating one of its parameters. We will use these and other modules to create a r.topmodel model and use R scripts including Isolated-Speciation-based Particle Swarm Optimization (ISPSO) (Cho et al. 2011), a particle swarm optimization algorithm in R, to calibrate its parameters.
...
<br>
Huidae /hidɛ/ is a water resources engineer. He received his master’s from Kyungpook National University and Ph.D. from Texas A&M University. He teaches Water Resources Engineering at New Mexico State University. He is a member of the GRASS Development Team and Project Steering Committee.
:::
:::
...
- GRASS is required on Linux, macOS, or Windows.
...
Cho (2000) rewrote TMOD9502.FOR and GRIDATB.FOR, the FORTRAN 77 version of TOPMODEL and the topographic index calculator by Beven, in C and integrated them with GRASS as the [r.topmodel](https://grass.osgeo.org/grass-stable/manuals/r.topmodel.html) and [r.topidx](https://grass.osgeo.org/grass-stable/manuals/r.topidx.html) modules, respectively. Both modules are included in the standard GRASS installation.
...
![](images/rastrigin.webp){.align-center}
...
- [Python 3](https://www.python.org/) installed with GRASS
...
![](images/streamstats-02331600-gauge.webp){.align-center width=75%}
...
![](images/streamstats-02331600-watershed.webp){.align-center width=75%}
...
![](images/earthexplorer-search-criteria.webp){.align-center width=75%}

We will use the [SRTM DEM](https://www2.jpl.nasa.gov/srtm/). Click Data Sets and search for "SRTM 1 arc-second".

![](images/earthexplorer-data-sets.webp){.align-center width=75%}
...
![](images/earthexplorer-results.webp){.align-center width=75%}
...
![](images/projpicker-watershed.webp){.align-center width=75%}

Select the first CRS. We will use [EPSG:2240](https://epsg.io/2240).

![](images/projpicker-epsg2240.webp){.align-center width=75%}

## Importing the DEM

Now that we know which CRS to use for our analysis, let's create a new GRASS project in EPSG:2240 and import the DEM by reprojecting it from its original CRS EPSG:4326 to EPSG:2240 on the fly. Open a new terminal.
...
![](images/outlet.webp){.align-center width=75%}
...
![](images/nationalmap-search.webp){.align-center width=75%}
...
![](images/nationalmap-download.webp){.align-center width=75%}
...
![](images/outlet-snapped.webp){.align-center width=75%}
...
![](images/streams-watershed.webp){.align-center width=75%}
...
![](images/watershed-lfp.webp){.align-center width=75%}
...
![](images/watershed-diff.webp){.align-center width=75%}
...
![Topidx Watershed](images/topidx-watershed.webp){.align-center width=75%}
...
![Subwatersheds and Suboutlets](images/subwatersheds-suboutlets.webp){.align-center width=75%}
...
The coordinates of NCDC weather stations are in latitude and longitude, so we need to create a new latlong project in [`EPSG:4326`](https://epsg.io/4326). Open a new terminal.
...
Import the watershed vector from the `epsg2240` project.

```bash
v.proj project=epsg2240 input=watershed
```
...
![Watershed Weather Stations](images/watershed-weather-stations.webp){.align-center width=75%}
...
```bash
v.proj project=epsg4326 input=prcp_stations_voronoi
v.proj project=epsg4326 input=evap_stations_voronoi
v.proj project=epsg4326 input=prcp_stations
v.proj project=epsg4326 input=evap_stations
```
...
From the `epsg4326` project, import all the files using [`import_usgs_pet.sh`](https://github.com/HuidaeCho/nsf-pose-2025-r.topmodel-workshop/blob/master/scripts/import_usgs_pet.sh).
...
![](images/evap-pet-boxplots.webp){width="75%" fig-align="center"}

![](images/evap-pet-time-series.webp){width="75%" fig-align="center"}
...
### Cumulative minimum of the objective function

![](images/cummin-obj.webp)
...
![](images/obs-c-sim-c-time-series.webp){.align-center width=75%}
...
![](images/obs-c-sim-c-ensemble.webp){.align-center width=75%}
...
![](images/obs-v-sim-v-time-series.webp){.align-center width=75%}
...
![](images/obj-obj-random.webp){.align-center width=75%}
...
![](images/x-obj-random.webp){.align-center width=75%}
...
![](images/x-obj.webp){.align-center width=75%}
